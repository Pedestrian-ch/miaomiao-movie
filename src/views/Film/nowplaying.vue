...
export default {
  ...
  activated () {
    if (this.cityId !== this.$store.state.city.cityId) {
      this.haveData = false
      this.axios.get('/ajax/movieOnInfoList?token=&ci=' + this.$store.state.city.cityId).then((res) => {
  ...
  methods: {
    ...
    handleToTouchEnd (pos, scroll) {
      if (pos.y < scroll.maxScrollY - 30) {
        var len = this.dataObject.movieList.length
        this.isLoader = true
        scroll.refresh()
        if (len !== this.dataObject.total) {
          this.axios
            .get(
              '/ajax/moreComingList?token=&ci=' + this.$store.state.city.cityId + '&limit=10&movieIds=' +
                encodeURI(
            ...
            .then((res) => {
              this.pullUpMsg = '加载完毕'
              this.dataObject.movieList.push(...res.data.coming)
              this.isLoader = false
              this.isPrompt = false
              setTimeout(() => {
                scroll.refresh()
